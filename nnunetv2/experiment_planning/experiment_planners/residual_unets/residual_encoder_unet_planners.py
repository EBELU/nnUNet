...
class SEAttentionResUNetPlanner(nnUNetPlannerResEncM):
    def __init__(self,
                 dataset_name_or_id,
                 gpu_memory_target_in_gb=8,
                 preprocessor_name='DefaultPreprocessor',
                 plans_name='nnUNetSEAttentionResUNetPlans',
                 overwrite_target_spacing=None,
                 suppress_transpose=False):
        ...
        self.UNet_class = ResidualUNet_SqueezeAttention

class CBAMAttentionResUNetPlanner(nnUNetPlannerResEncM):
    def __init__(self,
                 dataset_name_or_id,
                 gpu_memory_target_in_gb=8,
                 preprocessor_name='DefaultPreprocessor',
...
class SEGatedAttentionResUNetPlanner(nnUNetPlannerResEncM):
    def __init__(self,
                 dataset_name_or_id,
                 gpu_memory_target_in_gb=8,
                 preprocessor_name='DefaultPreprocessor',
                 plans_name='nnUNetSEGatedAttentionResUNetPlans',
                 overwrite_target_spacing=None,
                 suppress_transpose=False):
        ...
        self.UNet_class = ResidualUNet_SqueezeGatedAttention
...
class CBAMAttentionResUNetPlanner(nnUNetPlannerResEncM):
    def __init__(self,
                 ...
                 gpu_memory_target_in_gb=8,
                 preprocessor_name='DefaultPreprocessor',
                 plans_name='nnUNetCBAMAttentionResUNetPlans',
                 overwrite_target_spacing=None,
                 suppress_transpose=False):
...
class CBAMAttentionResUNetSmallPlanner(nnUNetPlannerResEncM):
    def __init__(self,
                 ...
                 gpu_memory_target_in_gb=8,
                 preprocessor_name='DefaultPreprocessor',
                 plans_name='nnUNetCBAMAttentionResUNetSmallPlans',
                 overwrite_target_spacing=None,
                 suppress_transpose=False):
    ...
    def determine_fullres_target_spacing(self) -> np.ndarray: # == OVERLOAD ==
        """
...
class CBAMAttentionResUNetE2SmallPlanner(CBAMAttentionResUNetSmallPlanner):
    def __init__(self,
                 ...
                 gpu_memory_target_in_gb=8,
                 preprocessor_name='DefaultPreprocessor',
                 plans_name='nnUNetCBAMAttentionResUNetE2SmallPlans',
                 overwrite_target_spacing=None,
                 suppress_transpose=False):
    ...
    def determine_fullres_target_spacing(self) -> np.ndarray: # == OVERLOAD ==
        ...
        target = np.percentile(spacings, 10, 0)

        # todo sizes_after_resampling = [compute_new_shape(j, i, target) for i, j in zip(spacings, sizes)]

        target_size = np.percentile(np.vstack(sizes), 10, 0)
        # we need to identify datasets for which a different target spacing could be beneficial. These datasets have
        # the following properties:
        # - one axis which much lower resolution than the others
        # - the lowres axis has much less voxels than the others
