...
class TverskyLoss(nn.Module):
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        if self.focal_gamma != 1:
            tversky = (1 - tversky) ** self.focal_gamma
        if not self.do_bg:
            if self.batch_dice:
                tversky = tversky[1:]
            else:
...
class MemoryEfficientSoftTverskyDiceLoss(nn.Module):
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        tversky = (intersect / (intersect + self.beta * FN + self.alpha * FP + float(self.smooth))).clamp_min(1e-8)
        if self.focal_gamma != 1:
            tversky = 1 - (1 - tversky) ** self.focal_gamma
